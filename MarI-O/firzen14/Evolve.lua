-- MarI/O by SethBling and Firzen 14
-- Feel free to use this code, but please do not redistribute it.
-- Intended for use with the BizHawk emulator and Super Mario World or Super Mario Bros. ROM.

-- modified fitness value to counrt blocks seen, not distance
...
function generateName()
        ...
        local name = "";
        local syls = math.random(2)+1;
        for syl=0,syls do
                name = name .. consonants[math.random(19)] .. vocals[math.random(5)]
        end
        return name
end
...
function displayGenome(genome)
        ...
        messageCell.x = 75
        messageCell.y = 105
        messageCell.value = network.neurons[Inputs].value
        cells[Inputs] = messageCell
...
while true do
        ...
        if timeout + timeoutBonus <= 0 or marioY > 0x1B0 then
        ...
        if pool.species[pool.currentSpecies].name == nil then
                pool.species[pool.currentSpecies].name = generateName();
        end
        if not forms.ischecked(hideBanner) then
                ...
                gui.drawText(0, 12, "Fitness: " .. math.floor(totalSeen - (pool.currentFrame) / 2 - (timeout + timeoutBonus)*2/3), 0xFF000000, 11)
